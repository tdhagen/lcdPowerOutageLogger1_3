...
#define OFFICIAL_ZENITH 90.83333  // for sunrise/set functions
#define CIVIL_ZENITH 96
#define NAUTICAL_ZENITH 102
#define ASTRONOMICAL_ZENITH 108
//#define LONG -83.6167    //longitude, Grand Blanc, MI, W083°37'
//#define LAT 42.9333      //latitude, Grand Blanc, MI, N42°56'
#define LONG -83.5103    //longitude, Davison, MI, W -83.510272
#define LAT 43.0552      //latitude, Davison, MI, N43.055259
//#define LONG -83.9378    //longitude, Hawks, MI, W-83.937840
//#define LAT 45.2507      //latitude, Hawks, MI, N45.250724

#define UTC_OFFSET -4    //US Eastern time  was 5/ daylight savings = 4
...
Timezone Eastern(EDT, EST);  // rule index #0
TimeChangeRule CDT = {
  "CDT", Second, Sun, Mar, 2, -300};    //Daylight time = UTC - 5 hours
TimeChangeRule CST = {
  "CST", First, Sun, Nov, 2, -360};     //Standard time = UTC - 6 hours
Timezone Central(CDT, CST);  //  rule index #1
TimeChangeRule MDT = {
  "MDT", Second, Sun, Mar, 2, -360};    //Daylight time = UTC - 6 hours
TimeChangeRule MST = {
  "MST", First, Sun, Nov, 2, -420};     //Standard time = UTC - 7 hours
Timezone Mountain(MDT, MST);  //  rule index  #2
TimeChangeRule PDT = {
  "PDT", Second, Sun, Mar, 2, -420};    //Daylight time = UTC - 7 hours
TimeChangeRule PST = {
  "PST", First, Sun, Nov, 2, -480};     //Standard time = UTC - 8 hours
Timezone Pacific(PDT, PST);  // rule index #3
TimeChangeRule utcRule = {
  "UTC", First, Sun, Nov, 2, 0};    //No change for UTC
Timezone UTC(utcRule, utcRule);  // rule index #4
Timezone *timezones[] = { 
  &Eastern, &Central, &Mountain, &Pacific, &UTC };
...
int timeOffset;  // holds the offset from UTC for current timezone rule, used in sunrise/set
int d;  //ordinal date for sunrise
byte ohour, ominutes, loopCount;
...
enum {
  RUN, DISP_OUTAGE, SET_START, SET_TZ, SET_CALIB, SET_YR, SET_MON, SET_DAY, SET_HR, SET_MIN, SET_SEC, SET_END};    //state machine states
...
void loop(void)
{
  ...
  case RUN:
    ...
    if (utc != lastUTC) {
      ...
      lcdDateTime(); // updates display each time thru loop
      loopCount++;  //  new to slow alternating
      if(loopCount >9)  //  this does the counting for how lon to display sunrise/set
      {
        loopCount=0;
      }
      brAdjust();
...
enum {
  VAL_MONTH, VAL_DAY, VAL_CALIB, VAL_TZ, VAL_SEC, VAL_OTHER};
uint8_t setType;
enum {
  WAIT, INCR, DECR, ZERO};
